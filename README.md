...
| 🎯 فحص مستهدف | `--repo OWNER/NAME` يفحص مستودعاً واحداً محدداً فقط (يدعم الاسم المختصر أيضاً) بدون لمس البقية. |
...
3. شغّل الوكيل:
   ...
   # فحص مستودع واحد محدد فقط (معاينة أو فعلي)
   python agent.py --repo elazamey/cela.1 --dry-run
   python agent.py --repo elazamey/cela.1
   ```
